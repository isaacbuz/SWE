...
# Configuration
NAMESPACE="${NAMESPACE:-piehr-staging}"
GKE_CLUSTER="${GKE_CLUSTER:-piehr-gke-staging}"
...
cd "$PROJECT_ROOT/infrastructure/kubernetes/overlays/staging"
...
# kubectl create job --from=cronjob/migrate-db migrate-db-$(date +%s) -n "$NAMESPACE"
...
# curl -f "https://$API_URL/api/projects" || exit 1
