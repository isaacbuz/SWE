...
export class EnhancedRateLimiter {
  ...
  async getUsage(userId: string): Promise<UsageStats> {
    ...
    const requestsToday = await this.storage.getCount(
      `requests:user:${userId}:day:${today}`,
    );
    const requestsThisMonth = await this.storage.getCount(
      `requests:user:${userId}:month:${month}`,
    );
