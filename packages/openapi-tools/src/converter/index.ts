...
function convertOperationToToolSpec(
  ...
  if (!operation.operationId) {
    console.warn(
      `Skipping operation ${method} ${path}: missing operationId`
    );
    return null;
  }
  ...
  // Extract description
  const description =
    operation.description || operation.summary || `Execute ${operation.operationId}`;

  // Extract category from first tag
  const category = operation.tags?.[0];

  // Build JSON Schema for parameters
  const jsonSchema = buildParameterSchema(
    operation,
    schemaConverter
  );
