...
**Mission:** Operate a software engineering company where the _workforce is AI agents_. We combine Anthropic (Claude) and OpenAI models with open-source coders (Qwen/Granite) under a **Mixture-of-Experts (MoE)** router, and ship changes as **GitHub PRs** guarded by compliance and test gates.

**Key Ideas**

- Contract-driven: every step emits typed JSON (plans, patch specs, reports).
- Provider-agnostic: the same tool surface is exposed to _both_ providers.
