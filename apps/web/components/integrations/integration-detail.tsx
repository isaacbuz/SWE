    ...
    <div className="rounded-lg border border-border-subtle bg-surface-secondary p-6">
          ...
          <div className="mt-2">
            <div className="flex items-center justify-between text-sm mb-1">
              ...
              <span className="text-ink-primary font-medium">
                {integration.rateLimit.remaining.toLocaleString()} /{" "}
                {integration.rateLimit.limit.toLocaleString()} remaining
              </span>
            </div>
            <div className="w-full bg-surface-tertiary rounded-full h-2">
              <div
                ...
                style={{
                  width: `${(integration.rateLimit.remaining / integration.rateLimit.limit) * 100}%`,
                }}
              />
            </div>
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {integration.credentials?.lastUpdated && (
              <span className="text-xs text-ink-tertiary">
                Updated{" "}
                {integration.credentials.lastUpdated.toLocaleDateString()}
              </span>
            )}
          </div>
          ...
          <div className="space-y-2">
            {integration.tools.map((tool) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    onToggleTool?.(integration.id, tool.id, !tool.enabled)
                  }
      ...
      {integration.usage && (
        <div className="border-t border-border-subtle pt-4">
          <span className="text-sm font-medium text-ink-primary mb-2 block">
            Usage Statistics
          </span>
          <div className="space-y-1 text-sm">
            <div className="flex justify-between">
              ...
              <span className="text-ink-primary font-medium">
                {integration.usage.last7Days} calls
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-ink-tertiary">Last 30 days:</span>
              <span className="text-ink-primary font-medium">
                {integration.usage.last30Days} calls
              </span>
            </div>
          </div>
        </div>
