          ...
          {currentProvider && (
            <div
              className={cn(
                ...
                getStatusColor(currentProvider.status),
              )}
            />
          )}
        </div>
          ...
          <>
            <div className="mb-3">
              ...
              <div className="text-xs text-ink-tertiary">
                Status:{" "}
                {currentProvider.status === "healthy"
                  ? "🟢 Healthy"
                  : currentProvider.status === "degraded"
                    ? "🟡 Degraded"
                    : "🔴 Down"}
              </div>
            </div>
        ...
        <div className="rounded-lg border border-border-subtle bg-surface-secondary p-4">
          ...
          <div className="space-y-2 mb-4">
            ...
            <div className="text-sm">
              ...
              <span className="text-ink-primary font-medium">
                {(lastExecution.duration / 1000).toFixed(1)}s
              </span>
            </div>
          </div>
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <button
                        onClick={() =>
                          setExpandedTool(
                            expandedTool === toolCall.id ? null : toolCall.id,
                          )
                        }
            ...
            <div className="space-y-1 text-xs">
              <div className="flex justify-between">
                ...
                <span className="text-ink-primary">
                  {lastExecution.tokensIn.toLocaleString()} in /{" "}
                  {lastExecution.tokensOut.toLocaleString()} out
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-ink-tertiary">Cost:</span>
                <span className="text-ink-primary font-medium">
                  ${lastExecution.cost.toFixed(4)}
                </span>
              </div>
            </div>
              ...
              <button
                onClick={() => {
                  ...
                  const alternative = providers.find(
                    (p) => p.id !== lastExecution.provider.id,
                  );
                  if (alternative) {
