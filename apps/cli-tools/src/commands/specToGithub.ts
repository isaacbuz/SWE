...
import { GitHubToolWrapper, CredentialVault } from "@ai-company/external-api-tools";
...
class SimpleLLMProvider {
  ...
  async complete(
    ...
    if (lastMessage.content.includes("specification") || lastMessage.content.includes("spec")) {
...
async function specToGithub(options: SpecToGithubOptions): Promise<void> {
  ...
  try {
    ...
    console.log(chalk.green(`✅ Loaded specification (${spec.length} characters)`));
    ...
    console.log(chalk.green(`✅ Loaded ${toolRegistry.getToolSpecs().length} tools`));
    ...
    toolExecutor.registerTool("createIssues", async (args: any) => {
      if (options.dryRun) {
        console.log(chalk.yellow(`[DRY RUN] Would create ${args.tasks.length} issues`));
        return {
          success: true,
...
`;
    ...
    const result = await pipeline.execute(prompt, "You are a helpful technical assistant.");
